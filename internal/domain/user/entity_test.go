...
func (suite *EntityTestSuite) TestNewUser() {
	suite.Run("Success", func() {
		...
		suite.Equal(UserStatusActive, got.Status)
...
func (suite *EntityTestSuite) TestUser_Rebuild() {
	...
	suite.Run("Success Rebuild", func() {
		newIsAdmin := true
		err := u.Rebuild("Jane", "Smith", "jane@example.com", UserStatusSuspended, &newIsAdmin)
		suite.NoError(err)
		suite.Equal("Jane", u.FirstName)
		suite.Equal("Smith", u.LastName)
		suite.Equal("jane@example.com", u.Email.String())
		suite.Equal(UserStatusSuspended, u.Status)
	...
	suite.Run("Invalid Email", func() {
		err := u.Rebuild("Jane", "Smith", "invalid", UserStatusActive, nil)
		suite.Error(err)
	})
}
